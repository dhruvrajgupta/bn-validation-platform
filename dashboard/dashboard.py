...
with super_model:
    ...
    selected_gt_model = st.selectbox("Select a ground truth model", model_names, key="select_gt_models")
                ...
                with st.form(key="save-network", border=False):
                    ...
                    if submitted:
                        ...
                        if name and type:
                            ...
                            if status == "Present":
                                st.caption(
                                    ":red[A model already exists with the same name, Please choose a different name for the model.]")
                            elif status == "Failed":
