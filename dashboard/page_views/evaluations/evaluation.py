...
if not model:
    ...
    with st.container(border=True):
        ...
        with st.container(border=True):
            ...
            if st.checkbox(f"**[Augmentation] Node Type, Observability, Node Label, Description, States Descriptions and Causal Factors**"):
                evaluation_name = f"type1_node_id_node_type_observability_node_labels_descriptions_state_descriptions_causal_factors_causalverb_{selected_causal_verb}"
                from utils.evaluation_functions_type1_options2 import node_id_node_type_observablity_node_labels_descriptions_states_descriptions_causal_factors
                evaluation_function = node_id_node_type_observablity_node_labels_descriptions_states_descriptions_causal_factors
                trigger_evaluation(evaluation_function, evaluation_name)

            if st.checkbox(f"**[Augmentation] Node Type, Observability, Node Label, Description, States Descriptions, Causal Factors and LLM inherent Evidences**"):
                evaluation_name = f"type1_node_id_node_type_observability_node_labels_descriptions_state_descriptions_causal_factors_evidences_causalverb_{selected_causal_verb}"
                from utils.evaluation_functions_type1_options2 import node_id_node_type_observablity_node_labels_descriptions_states_descriptions_causal_factors_evidences
            ...
            if st.checkbox(f"**[Augmentation] Node Type, Observability, Node Label, Description, States Descriptions, Causal Factors, LLM inherent Evidences, Entites & Relations and Causalities extracted from Guideline pages**"):
                evaluation_name = f"type1_node_id_node_type_observability_node_labels_descriptions_state_descriptions_causal_factors_evidences_ent_rel_causalities_causalverb_{selected_causal_verb}"
                from utils.evaluation_functions_type1_options2 import node_id_node_type_observablity_node_labels_descriptions_states_descriptions_causal_factors_evidences_page_ent_rel_causalities
                evaluation_function = node_id_node_type_observablity_node_labels_descriptions_states_descriptions_causal_factors_evidences_page_ent_rel_causalities
                trigger_evaluation(evaluation_function, evaluation_name)

            if st.checkbox(f"**[Augmentation]** Only Node Label"):
                evaluation_name = f"type1_node_labels_causalverb_{selected_causal_verb}"
                from utils.evaluation_functions_type1_options2 import node_labels_causal_verb
                evaluation_function = node_labels_causal_verb
                trigger_evaluation(evaluation_function, evaluation_name)

            if st.checkbox(f"**[Augmentation]** Only Node Label and Causal Factors"):
                evaluation_name = f"type1_node_labels_causal_factors_causalverb_{selected_causal_verb}"
                from utils.evaluation_functions_type1_options2 import node_labels_causal_factors_causal_verb
        ...
        with st.container(border=True):
            ...
            evaluation_name = f"type2_node_id__node_type_observability_labels_description_states_descriptions"
            if st.checkbox(
                    f"**[Augmentation] Node Type, Observability, Node Label, Description and States Descriptions**",
                    key=evaluation_name):
                ...
                evaluation_function = node_id_node_type_observability_label_description_states_descriptions
                trigger_evaluation(evaluation_function, evaluation_name)

            evaluation_name = f"type2_node_id__node_type_observability_labels_description_states_descriptions_causal_factors"
            if st.checkbox(
                    f"**[Augmentation] Node Type, Observability, Node Label, Description, States Descriptions and Causal Factors**",
                    key=evaluation_name):
                from utils.evaluation_functions_type2_options2 import \
                    node_id_node_type_observability_label_description_states_descriptions_causal_factors
                evaluation_function = node_id_node_type_observability_label_description_states_descriptions_causal_factors
                trigger_evaluation(evaluation_function, evaluation_name)

            evaluation_name = f"type2_node_id__node_type_observability_labels_description_states_descriptions_causal_factors_evidences"
            if st.checkbox(
                    f"**[Augmentation] Node Type, Observability, Node Label, Description, States Descriptions, Causal Factors and LLM inherent Evidences**",
                    key=evaluation_name):
                from utils.evaluation_functions_type2_options2 import \
            ...
            evaluation_name = f"type2_node_labels_causal_factors_only"
            if st.checkbox(f"**[Augmentation]** Only Node Label and Causal Factors", key=evaluation_name):
                from utils.evaluation_functions_type2_options2 import node_label_causal_factors
