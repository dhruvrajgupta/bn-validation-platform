            ...
            for node in nodes:
                ...
                if st.checkbox(f"{status_icon} {node}", key=f"gt-{node}-desc"):
                    if node_info_db:
                        node_info = {
                            'node_id': node,
                            'states': gt_model_bn.states[node],
                        ...
                        if node_info['entity_information']:
                            ...
                            for id, entity_dict in enumerate(node_info['entity_information']):
                                if entity_dict['ontology_name'] == "MeSH":
                                    node_info['entity_information'][id]['link'] = MESH_LINK + urllib.parse.urlencode({"q": entity_dict['label']})
