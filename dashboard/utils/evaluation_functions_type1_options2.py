...
#### EVALUATION WITH NODE ID THEIR STATE NAMES, NODE TYPE, OBSERVABILITY, NODE LABELS AND DESCRIPTIONS ####
NODE_ID_NODE_TYPE_OBSERVABILITY_NODE_LABELS_DESCRIPTIONS_STATE_DESCRIPTIONS_CAUSAL_FACTORS_EVIDENCES_PAGES_ENT_REL_DIFF_CAUSAL_VERB = """\
edge - The edge which needs to be verified.
explanation - Explanation of what the edge represents and its validity.
causal_direction - Either Positive or Negative or Unknown. A positive influence direction indicates that both factors change in the same direction (e.g. an increase causes an increase effect). A negative influence direction indicates the opposite changes (e.g. an increase causes a decrease effect).
causal_factor - Is necessary or sufficient condition for an effect to occur. Exposure is a term commonly used in epidemiology to denote any condition that is considered as a possible cause of disease. Exposure is considered necessary when it always precedes the effects (e.g. symptoms) and always presents when the effects occur. A sufficient cause is a causal factor whose presence or occurrence guarantees the occurrence of symptom.
...
def node_id_node_type_observablity_node_labels_descriptions_states_descriptions_causal_factors_evidences_page_ent_rel(incorrect_edges, eval_name, llm_model_name, bn_model):
    causal_verb = eval_name.split("_")[-1]
    ...
    create_dataset(incorrect_edges, NODE_ID_NODE_TYPE_OBSERVABILITY_NODE_LABELS_DESCRIPTIONS_STATE_DESCRIPTIONS_CAUSAL_FACTORS_EVIDENCES_PAGES_ENT_REL_DIFF_CAUSAL_VERB)
...
#### EVALUATION WITH NODE ID THEIR STATE NAMES, NODE TYPE, OBSERVABILITY, NODE LABELS AND DESCRIPTIONS ####
NODE_ID_NODE_TYPE_OBSERVABILITY_NODE_LABELS_DESCRIPTIONS_STATE_DESCRIPTIONS_CAUSAL_FACTORS_EVIDENCES_PAGES_ENT_REL_CAUSALITIES_DIFF_CAUSAL_VERB = """\
edge - The edge which needs to be verified.
explanation - Explanation of what the edge represents and its validity.
causal_direction - Either Positive or Negative or Unknown. A positive influence direction indicates that both factors change in the same direction (e.g. an increase causes an increase effect). A negative influence direction indicates the opposite changes (e.g. an increase causes a decrease effect).
causal_factor - Is necessary or sufficient condition for an effect to occur. Exposure is a term commonly used in epidemiology to denote any condition that is considered as a possible cause of disease. Exposure is considered necessary when it always precedes the effects (e.g. symptoms) and always presents when the effects occur. A sufficient cause is a causal factor whose presence or occurrence guarantees the occurrence of symptom.
...
def node_id_node_type_observablity_node_labels_descriptions_states_descriptions_causal_factors_evidences_page_ent_rel_causalities(incorrect_edges, eval_name, llm_model_name, bn_model):
    causal_verb = eval_name.split("_")[-1]
    ...
    create_dataset(incorrect_edges, NODE_ID_NODE_TYPE_OBSERVABILITY_NODE_LABELS_DESCRIPTIONS_STATE_DESCRIPTIONS_CAUSAL_FACTORS_EVIDENCES_PAGES_ENT_REL_CAUSALITIES_DIFF_CAUSAL_VERB)
...
#### EVALUATION WITH ONLY NODE LABEL ####
ONLY_NODE_LABELS_DIFF_CAUSAL_VERB = """\
Among these two options which one is the most likely true:
...
def node_labels_causal_verb(incorrect_edges, eval_name, llm_model_name, bn_model):
    causal_verb = eval_name.split("_")[-1]
    ...
    create_dataset(incorrect_edges, ONLY_NODE_LABELS_DIFF_CAUSAL_VERB)
...
#### EVALUATION WITH ONLY NODE LABEL and CAUSAL FACTORS ####
ONLY_NODE_LABELS_CAUSAL_FACTORS_DIFF_CAUSAL_VERB = """\
edge - The edge which needs to be verified.
explanation - Explanation of what the edge represents and its validity.
causal_direction - Either Positive or Negative or Unknown. A positive influence direction indicates that both factors change in the same direction (e.g. an increase causes an increase effect). A negative influence direction indicates the opposite changes (e.g. an increase causes a decrease effect).
causal_factor - Is necessary or sufficient condition for an effect to occur. Exposure is a term commonly used in epidemiology to denote any condition that is considered as a possible cause of disease. Exposure is considered necessary when it always precedes the effects (e.g. symptoms) and always presents when the effects occur. A sufficient cause is a causal factor whose presence or occurrence guarantees the occurrence of symptom.
...
def node_labels_causal_factors_causal_verb(incorrect_edges, eval_name, llm_model_name, bn_model):
    causal_verb = eval_name.split("_")[-1]
    ...
    create_dataset(incorrect_edges, ONLY_NODE_LABELS_CAUSAL_FACTORS_DIFF_CAUSAL_VERB)
